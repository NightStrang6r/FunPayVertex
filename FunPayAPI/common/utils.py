...
def get_currency_code(currency_symbol: str) -> str | None:
    if currency_symbol == "₽":
        return "RUB"
    if currency_symbol == "$":
        return "USD"
...
class RegularExpressions(object):
    ...
    def __init__(self):
        ...
        self.REFUND_BY_ADMIN = re.compile(r"Администратор [a-zA-Z0-9]+ вернул деньги покупателю [a-zA-Z0-9]+ по заказу #[A-Z0-9]{8}\.")
        """
        ...
        self.SCAM_WARNING = "Уважаемые продавцы, не доверяйте сообщениям в чате! " \
                            "Перед выполнением заказа всегда проверяйте наличие оплаты в разделе «Мои продажи»."
        """
        Точный текст первого сообщения FunPay.
        """

        self.PRODUCTS_AMOUNT = re.compile(r"((?:\d+\s*)+ шт\.)")
        """
